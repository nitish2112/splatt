  ...
  idx_t * nunique)
{
  ...
  *nunique = found;
  ...
  idx_t * slices = malloc(found * sizeof(*slices));
  idx_t ptr = 0;
...
idx_t * tt_get_hist(
  ...
  idx_t const mode)
{
  idx_t * restrict hist = malloc(tt->dims[mode] * sizeof(*hist));
  ...
  sptensor_t * const tt)
{
  ...
  /* slice counts */
  idx_t * scounts = malloc(maxdim * sizeof(*scounts));
  ...
  for(idx_t m=0; m < nmodes; ++m) {
    ...
    tt->indmap[m] = malloc(dim_sizes[m] * sizeof(**tt->indmap));
...
sptensor_t * tt_alloc(
  ...
  idx_t const nmodes)
{
  sptensor_t * tt = (sptensor_t*) malloc(sizeof(*tt));
  tt->tiled = SPLATT_NOTILE;

  tt->nnz = nnz;
  tt->vals = malloc(nnz * sizeof(*tt->vals));

  tt->nmodes = nmodes;
  tt->type = (nmodes == 3) ? SPLATT_3MODE : SPLATT_NMODE;

  tt->dims = malloc(nmodes * sizeof(*tt->dims));
  tt->ind  = malloc(nmodes * sizeof(*tt->ind));
  for(idx_t m=0; m < nmodes; ++m) {
    tt->ind[m] = malloc(nnz * sizeof(**tt->ind));
    tt->indmap[m] = NULL;
  }
